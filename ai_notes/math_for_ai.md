...
如果我们有两个不相关的随机变量X, Y, 则
$$
V[X+Y] = V[X] + V[Y]
$$
如果我们定义X,Y之间的内积为二者的协方差$<X,Y>:=Cov[X,Y]$, 则随机变量的长度为它的标准差
$$
||X|| = \sqrt{Cov[X,X]} = \sqrt{V[X]} = \sigma[X]
$$
随机变量越“长”, 其不确定性越大

两个随机变量之间的角度为它俩的相关系数
$$
cos\theta = \frac{<X,Y>}{||X||||Y||} = \frac{Cov[X,Y]}{\sqrt{V[X]V[Y]}}
$$
也就是当角度越大的时候, 它们的相关性从正相关到不相关到负相关.



## 高斯分布

高斯分布是所有连续型随机变量的概率分布中被研究的最透彻的一种分布, 它也被叫做正态分布.

它的重要性事实上来源于一些便于计算的性质, 在之后的定义线性回归的似然和先验时会用到
...
$$
多元高斯分布(Multivariate Gaussian distribution)完全由均值向量$\mu$和协方差矩阵$\Sigma$ 描述
$$
p(x|\mu, \Sigma) = (2\pi)^{-\frac{D}{2}}|\Sigma|^{-1/2}exp(-1/2(x-\mu)^T\Sigma^{-1}(x-\mu))
$$


### 高斯分布的边际分布和条件分布仍是高斯分布
...
指数族的分布类在保持一般性的同时, 也保留了有利的计算和推断特性,

在介绍指数族之前, 先简单看看指数族中三个“有名”的成员: 伯努利分布, 二项分布和贝塔分布.

#### 例6.8 伯努利分布

伯努利分布针对单变量二元随机变量X的分布, 其状态为$x\in\{0, 1\}$, 它由一个连续参数$\mu\in [0, 1]$控制(这个$\mu$就是x为1的概率), 则伯努利分布$Ber(\mu)$定义为
$$
p(x|\mu) = \mu^x(1-\mu)^{1-x} \ \ \  其实就是 \mu 或 1-\mu \\
E[x] = \mu
V[x] = \mu(1-\mu)
$$
